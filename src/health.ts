...
import { computeReconciledBundles, semverCompare } from './install.ts'
...
export function packageInstalled(profileDir: string, name: string): boolean {
...
export function readPackageManifest(profileDir: string, name: string): Record<string, unknown> | null {
 ...
 * walks both).
 */
function peerVersion(profileDir: string, peer: string): string | null {
...
function tryResolvePeer(peer: string): boolean {
  try {
...
export function satisfiesRange(installed: string | null, range: string): boolean {
  ...
  if (trimmed.startsWith('>=')) return semverCompare(installed, trimmed.slice(2)) >= 0
  if (trimmed.startsWith('<=')) return semverCompare(installed, trimmed.slice(2)) <= 0
  if (trimmed.startsWith('>')) return semverCompare(installed, trimmed.slice(1)) > 0
  if (trimmed.startsWith('<')) return semverCompare(installed, trimmed.slice(1)) < 0
  if (/^\d+\.\d+\.\d+/.test(trimmed)) return semverCompare(installed, trimmed.split(/\s/)[0] ?? trimmed) === 0
 ...
 */
export function collectHealth(profileDir: string, profileName: string): HealthPayload {
  ...
  for (const bundle of [...new Set([...bundles, ...Object.keys(dependencies)])]) {
    if (bundle.startsWith('@deepseek-ai/')) continue // in-box stack lives elsewhere
  ...
  for (const [name] of Object.entries(dependencies)) {
    if (!packageInstalled(profileDir, name)) continue
    const own = readPackageManifest(profileDir, name)
    const peers = own?.peerDependencies as Record<string, string> | undefined
    if (peers === undefined) continue
    for (const [peer, range] of Object.entries(peers)) {
      if (HOST_PROVIDED_PEER.test(peer)) continue
  ...
  } catch { /* keep the on-disk list as the preview */ nextBundles = [...bundles] }
