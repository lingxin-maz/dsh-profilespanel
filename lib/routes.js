...
import { readJsonBody, sameOrigin, sendJson, trustedLoopbackRequest } from './http.js';
import { installIntoProfiles, listInstallableProfiles, undoProfile, validateInstallTarget, } from './install.js';
import { detectDesktop, resolveInstallTargets, desktopAppDataDir, readDesktopSelection } from './desktop.js';
import { buildInstallPreview, buildGithubPreview } from './registry.js';
import { alignAcrossProfiles, collectUpdates, updateIntoProfiles } from './updates.js';
import { waitForNewerOrTimeout } from './events.js';
import { collectHealth } from './health.js';
import { attributeBundle, isHotReloadable } from './attribution.js';
import { diffProfiles } from './compare.js';
import { appendAudit, readAudit } from './audit.js';
import { manifestBundles, manifestDeps, readProfileManifest, } from './profile.js';
...
export function buildStatus(host) {
    ...
    if (state.boot !== null) {
        ...
        try {
            ...
            const attr = (name, index) => attributeBundle({
                name,
                index,
                dependencies,
            ...
            for (const [index, name] of disk.entries()) {
                if (!seen.has(name))
                    bundles.push({ name, state: 'pending', ...attr(name, index) });
            }
        }
        catch (error) {
    ...
    const desktopRuntime = host.ctx.get('desktopRuntime');
    ...
    const desktopProfiles = host.ctx.get('desktopProfiles');
...
export function mountPanelRoutes(host) {
    ...
    const recordAudit = (action, rows, pkg, spec) => {
    ...
    const disposers = [
        ...
        host.webServer.register({
            ...
            handler: (request, response) => {
                ...
                setTimeout(() => {
                    void Promise.resolve(desktopRuntime.requestRestart()).catch((error) => {
                        host.state.restarting = false;
            ...
            handler: async (request, response) => {
                ...
                if (host.hotReload !== undefined) {
                    ...
                    if (result.ok)
                        ...
                        sendJson(response, 501, { error: result.reason ?? 'hot reload unavailable' });
                    return;
                }
                const hmr = host.ctx.get('hmr');
